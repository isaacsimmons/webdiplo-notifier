...
class GameParser(HTMLParser):
    ...
    ALERT_THRESHOLD = 900 #900s = 15 minutes
    #TODO: change this to better accommodate polling rate? long-running process could "plan" when it wakes up instead
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'span' and len(attrs) == 3 and len(attrs[0]) == 2 and attrs[0][0] == 'class' and attrs[0][1] == 'timeremaining':
            self.games[-1]['time_left'] = (int(attrs[1][1]) - int(attrs[2][1])) / 60
        ...
        if tag == 'img' and len(attrs) == 3 and len(attrs[1]) == 2 and attrs[1][0] == 'alt':
            ...
            if attrs[1][1] == 'Completed':
                self.current_country['status'] = attrs[1][1]
        if tag == 'img' and len(attrs) == 3 and len(attrs[1]) == 2 and attrs[1][0] == 'alt' and attrs[1][
            1] == 'Unread message':
    ...
    def extract_alert_status(self, games):
        ...
        for game in games:
            ...
            #about to miss turn
            game_alerts['timeout'] = int(game['time_left']) < self.ALERT_THRESHOLD and own_status == 'Not received'
